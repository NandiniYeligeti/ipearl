  ...
  >
    <div className="max-w-7xl mx-auto px-4 py-16 lg:py-24">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        >
          {/* Hero Text */}
          <div className="min-h-48 flex flex-col justify-start gap-4">
            <motion.div
              key={`slide-${currentSlide}`}
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5 }}
              className="bg-white/20 backdrop-blur-sm p-8 rounded-3xl shadow-xl"
            >
              <h1 className="text-5xl lg:text-6xl font-bold text-slate-800 leading-tight">
                {heroSlides[currentSlide].title}
              </h1>

              <p className="text-2xl text-slate-700 font-semibold mt-2">
          ...
          <div className="bg-white/20 backdrop-blur-sm p-6 rounded-3xl shadow-xl">
            <p className="text-sm font-semibold text-slate-700 uppercase tracking-widest mb-4">
              Choose Your Color
            </p>
            ...
            <div className="flex flex-wrap gap-4">
              {colors.map((color) => (
                ...
                >
                  <div
                    className={`w-12 h-12 rounded-full shadow-lg ring-2 transition-all ${
                      currentColor === color.id
